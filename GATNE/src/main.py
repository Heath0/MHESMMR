...
def parse_args():
    ...
    parser.add_argument('--edge-dim', type=int, default=32,
                        help='Number of edge embedding dimensions. Default is 32.') # 边嵌入维度 {8, 16, 32, 64, 128}
...
def train_model(network_data, feature_dic, log_name):
    ...
    if feature_dic is not None:
        ...
        print('feature node2vec 128 dimension: ' + str(feature_dim))
...
if __name__ == "__main__":
    args = parse_args()
    file_name = r'../data/SM2miR3_exp/'
    print(args)

    #feature_dic = load_feature_data(file_name+r'feature/attri_line_128.txt')
    feature_dic = None

    log_name = file_name.split('/')[-1] + f'_evaltype_{args.eval_type}_b_{args.batch_size}_e_{args.epoch}'

    training_data_by_type = load_training_data(file_name+r'SM2miR3_exp_train.txt')
    ...
    final_model,index2word = train_model(training_data_by_type, feature_dic, log_name + '_' + time.strftime('%Y-%m-%d %H-%M-%S',time.localtime(time.time())))

    save_dict(file_name + 'experiment/feature GATNE non_attri 32edge down.csv',final_model)
